...
function HeroThemeWaves() {
  ...
  return <div aria-hidden="true" className="hero-theme-waves pointer-events-none absolute inset-x-0 bottom-1 z-30 h-5 overflow-hidden">
    <svg viewBox="0 0 1440 24" preserveAspectRatio="none" className="h-full w-full">
      <path d={redWave} fill="#d50b12"/>
      <path d={yellowWave} fill="#ffc400"/>
    </svg>
  </div>;
}
...
export function HomeHeroSlider() {
  ...
  return <section aria-label="Creative All Stars Academy highlights" className="relative min-h-[455px] overflow-hidden bg-[#031f66] text-white sm:min-h-[560px] lg:min-h-[540px]" onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}>
    ...
    <div className="container-shell relative z-10 flex min-h-[455px] items-start py-4 sm:min-h-[560px] sm:py-6 lg:min-h-[540px] lg:py-5">
      <div className="max-w-4xl pb-10 pt-1 sm:pb-20 sm:pt-0">
        <AnimatePresence mode="wait">
          <motion.div key={active} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:reduceMotion?0:.2}}>
            ...
            <motion.div initial={reduceMotion?{opacity:1}:{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={entrance(.58)} className="mt-4 grid grid-cols-2 gap-2 sm:mt-7 sm:flex sm:gap-3">
    ...
    <div className="absolute inset-x-0 bottom-1 z-20 sm:bottom-0 sm:border-t sm:border-white/15 sm:bg-[#020d2b]/55 sm:backdrop-blur-md">
