...
export default function BlogDetails() {
  ...
  return <div className="pb-12 sm:pb-24">
    ...
    <section className="mx-auto mt-6 grid max-w-4xl grid-cols-1 gap-6 px-4 sm:mt-8 sm:gap-12 sm:px-6 lg:px-8">
      ...
      {relatedPosts.length > 0 && <div className="space-y-6"><h3 className="text-xl font-extrabold text-blue-950">Related publications</h3><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-6">{relatedPosts.map((related) => <article key={related.id} className="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xs"><div className="relative h-24 bg-gray-50 sm:h-32"><Image src={related.featuredImage} alt={related.title} fill sizes="(min-width: 640px) 30vw, 50vw" className="object-cover"/></div><div className="flex flex-grow flex-col justify-between p-3 sm:p-4"><h4 className="line-clamp-2 text-xs font-extrabold text-blue-950 hover:text-blue-600"><Link href={`/blog/${related.id}`}>{related.title}</Link></h4><time dateTime={related.date} className="mt-2 block text-[10px] font-bold uppercase text-gray-400">{related.date}</time></div></article>)}</div></div>}
    </section>
  </div>;
}
