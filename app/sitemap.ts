...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const { blogPosts, classes, jobs } = await getPublicContent().catch(() => defaultPublicContent);
  ...
  const contentRoutes: MetadataRoute.Sitemap = [
    ...
    ...classes.map(item => ({ url: `${base}/classes/${item.id}`, lastModified: new Date(), changeFrequency: 'monthly' as const, priority: .6 })),
