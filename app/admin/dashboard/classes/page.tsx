    ...
    {classes.length === 0 ? <div className="rounded-3xl border border-dashed border-gray-300 bg-white p-10 text-center"><GraduationCap className="mx-auto h-8 w-8 text-gray-300"/><p className="mt-3 text-sm font-bold text-gray-500">No class records are available yet.</p></div> : <div className="grid gap-5 lg:grid-cols-2">
      {classes.map((item) => {
        ...
        return <article key={item.id} className={`rounded-3xl border bg-white p-6 shadow-xs ${editingId === item.id ? 'border-blue-400 ring-2 ring-blue-100' : 'border-gray-100'}`}>
          ...
          <div className="mt-5 flex items-center gap-3 rounded-2xl bg-gray-50 p-3">{teacher ? <><div className="grid h-10 w-10 place-items-center overflow-hidden rounded-full border-2 border-yellow-400 bg-blue-50">{teacher.image ? <img src={teacher.image} alt="" className="h-full w-full object-cover"/> : <UserRound className="h-4 w-4 text-blue-300"/>}</div><div><p className="text-[10px] font-bold uppercase text-gray-400">Lead educator</p><p className="text-xs font-extrabold text-blue-950">{teacher.name}</p></div></> : <><div className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200"><UserRound className="h-4 w-4 text-gray-400"/></div><div><p className="text-[10px] font-bold uppercase text-gray-400">Lead educator</p><p className="text-xs font-extrabold text-gray-500">Not assigned</p></div></>}</div>
