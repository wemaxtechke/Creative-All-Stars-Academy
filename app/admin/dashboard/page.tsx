...
export default function AdminDashboardMain() {
  ...
  return <div className="mx-auto max-w-7xl space-y-8">
    ...
    <section className="grid grid-cols-2 gap-3 sm:gap-5 xl:grid-cols-4">{metrics.map(({label,value,detail,icon:Icon,href,tone})=><Link href={href} key={label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg sm:p-6"><div className="flex items-start justify-between gap-2"><div><p className="text-xs font-bold text-slate-500 sm:text-sm">{label}</p><p className="mt-2 text-2xl font-extrabold text-[#0b1f3a] sm:mt-3 sm:text-4xl">{value}</p></div><span className={`rounded-xl p-2 sm:p-3 ${tone}`}><Icon className="h-4 w-4 sm:h-5 sm:w-5"/></span></div><p className="mt-2 text-[10px] font-medium text-slate-400 sm:mt-3 sm:text-xs">{detail}</p></Link>)}</section>
